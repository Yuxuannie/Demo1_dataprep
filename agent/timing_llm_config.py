...
def initialize_timing_llm():
    ...
    # Set timing-optimized parameters
    timing_params = {
    ...
    logger.info(f"   Temperature: {os.getenv('LLM_TEMPERATURE')} (consistency optimized)")
    logger.info(f"   Top-P: {os.getenv('LLM_TOP_P')} (domain focused)")
    logger.info(f"   Max Tokens: {os.getenv('LLM_NUM_PREDICT')} (detailed explanations)")
    if applied_count > 0:
        logger.info(f"   Applied {applied_count} timing-specific parameters")
